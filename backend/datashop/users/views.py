...
from .models import LoginLog, Notification, User
...
from .utils import create_notification, get_client_ip, send_welcome_email
...
class ChangePinView(APIView):
    ...
    def post(self, request):
        ...
        if not user.pin_set:
            return Response(
                {"error": "PIN not set. Use /auth/pin/set/ first."}, status=400
            )
        serializer = ChangePinSerializer(data=request.data)
        if serializer.is_valid():
            try:
                verified = user.verify_transaction_pin(
